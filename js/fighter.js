...
(function (global) {
  ...
  const SPEARWIN = 0.55; // window (s) between an up-press and a jab to launch the SPEAR (generous)
  ...
  class Fighter {
    ...
    render(ctx, world) {
      ...
      const drawBody = (alpha) => {
        ...
        // render the fighter (incl. any custom drawn skin) live every frame, so the actual skin
        // always shows — no pose-cache that could blit a different fighter's/default cached bitmap.
